...
**RSX TV facts (RSX-SIGNAL-1 / 1.1):** Pine rolling detector emits `rsx_tv_div` (`bullish`/`bearish`) and `rsx_tv_pivot` (`high`/`low`). Times are closed-bar OpenTime ms. Divergence: `ConfirmedAt` = confirm bar, `AnchorAt` = prior bar. Pivot: `ConfirmedAt` = confirm bar, `AnchorAt` = two bars back. Projector paints arrow-only markers. Always published with the other RSX families (not gated by a UI selector).

**RSX fact visibility (RSX-VISIBILITY-1 ✅ frozen, `749912f`):** Visibility does not define factual existence. The five families (`rsx_tv_div`, `rsx_tv_pivot`, `rsx_zz_div`, `rsx_fractal_div`, `rsx_fractal_pivot`) coexist as independent facts. Frontend checkboxes only filter paint by annotation `source`. Visibility is not in the RSX fingerprint and does not POST/replay Jurik. Annotation repaint key is `annotationRevision` + `visibilityMask` (five bits) + `line_rsx` series identity. Do **not** reopen this chapter. Unused computation may later sleep based on consumer demand — that is not implemented here.

**RSX fractal facts (RSX-SIGNAL-3 ✅ frozen, `c856fef`):** `scanRSXFractalHits` + `CheckClassicDivergence` emit `rsx_fractal_div` (`Pattern` `class_a` / `class_b` / `class_c`) and `rsx_fractal_pivot` (`high` / `low`). Always published with the other RSX families. `AnchorAt` = fractal pivot OpenTime; `ConfirmedAt` = first closed bar that completes the fractal window (`pivot+PivotRadius` for divergence, `pivot+MacroPivotRadius` for P). Live/history share `FractalFactsAt` on that confirm bar only (O(lookback), not a prefix rescan). Class B paints arrow only; A/C add `A Bull` / `A Bear` / `C Bull` / `C Bear`. Fractal pivots are blue arrows. Old L/LL/S/SS/P are not public. Do **not** reopen detector math or lookback search.
...
**DAG-DEMAND-1:** PRESENTATION does not own computation. Per-Frame RSX analytical mask = WS plot union OR WS fact-source union OR proven internal demand (`Live`: `NeedRSXCore` for shadow Jurik). ChartOnly unused TFs: RSXCore / TV / Fractal / DAG ZigZag / ZZ collector all 0; OHLCV reducers stay on. Closure: `line_rsx`/`line_rsx_signal` → Core; either TV source → Core+TV; either Fractal source → Core+Fractal; `rsx_zz_div` → Core+DAG ZZ+collector. Core does not imply fact families. TV and Fractal are Frame scanners, not DAG nodes. DAG ZigZag (`rsx_zz_div`) is not Frame `a.zigzag` (fib/geometry). WS `facts` omitted/null → all families; `facts: []` → none (pointer presence, not `len==0`). One wake transaction has one RSX truth: family wake uses retained live Jurik if Core was already on; if Core wakes too, rebuild Jurik once and share that series. Fact sleep keeps historical events; wake rematerializes the retained closed window (replace family in-window, identity `Source,Direction,Pattern,ConfirmedAt,AnchorAt`). `/api/history` stays an independent replay. Detector identity is not selected by UI; visibility may create demand; zero demand may sleep factual compute.
...
**NEXT:** see `docs/OPEN_DEBTS.md` — primary: **#76 ScoreNodes**. Measure before MICRO-IDLE-1.

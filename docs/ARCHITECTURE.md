...
**Wozduh:** DAG bus only; Falcon Evaluate gated; legend = chrome only (no per-tick HTML metrics). **WOZDUH-WIRE-1:** live/history pack only subscribed Wozduh scalar plot IDs (visibility → chart subscription). Compute still runs all atoms. Enable hydrates the current store window before reveal. `woz_slow` stays on the wire while hidden (pane/crosshair owner).  

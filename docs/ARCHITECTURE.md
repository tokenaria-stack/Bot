...
13. **PaneLayout (ADR-019 + PANE-WORKSPACE-MAXIMIZE-1).** FE owns footer pane membership (`visible` / `order` / `footerHeights` px / `fullscreenPaneId`). Ind menu from Manifest HostIDs; persist ∩ manifest. Price always on (not a HostID). **LayoutController** applies CSS Grid, height drag, and legend reorder. `fullscreenPaneId` is workspace-maximize: `applyStack` uses one `minmax(0, 1fr)` track and one participating wrap inside `#charts-stack`. Layout suppression is not `visible[]`. No viewport-fixed overlay.

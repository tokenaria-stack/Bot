...
15. **Scale contribution (ADR-022 / #68 + WOZDUH-PANE-AUTOSCALE-OWNER-1).** What Auto *measures* is `autoscaleInfoProvider` from DDR `scaleContribution` (`dynamic` | `bounded` | `ignore`) or pane chrome that reuses the same translator. Wozduh bounded `[-5,105]` is pane-owned on the Extreme Bands private host. RSX bounded stays on `line_rsx`. ScaleController never owns oscillator domains.

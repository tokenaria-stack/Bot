...
**Wozduh:** DAG bus only; Falcon Evaluate gated; legend = chrome only (no per-tick HTML metrics). **WOZDUH-WIRE-1 frozen** (`0c2ecce`): live/history pack only subscribed Wozduh scalar plot IDs. **WOZDUH-ACTIVE-1A:** `/api/history` replay runs only the requested Wozduh compute closure; `ReplayClosedBars` default is still compute-all. Live Frame still runs all Wozduh streams. Enable hydrates the current store window before reveal. `woz_slow` stays on the wire while hidden (pane/crosshair owner).  

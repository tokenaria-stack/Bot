...
**HISTORY-IDLE-PUMP-1 (frozen):** viewport history demand is human-owned. A TimeCamera **user** VIEW commit may note `userNav` (`ChartAdapter.setUserViewCommit`). Paint, LWC range echo, and system restore must not schedule a new page. Post-flush consumes pending only. Sparse `sourceContinue` is separate. **HISTORY-ZOOM-OUT-EDGE-1 parked:** full zoom-out vibration at the loaded-island edge (5000+3000+3000 > 9000). Operator will not zoom fully out for strategy work.
...
12. **Timeline Recovery UX (ADR-018 + TIMELINE-RECOVERY-STATE-1).** FE `TimelineRecovery` owns LIVE↔HEALING plus `snapshotRequired`. Duplicate healing ignored. Watchdog only while Master is unpublishable. Browser reconnect is transport distrust, not HEALING. Dense `gapDetected` and reconnect share `requestDenseRecovery`. Current `timeline_state` (welcome + request) plus change events; TF subscribe does not echo state. Recovery completes only after authoritative `replaceMonolith`. Sparse MICRO-2B unchanged.
13. **PaneLayout (ADR-019 + PANE-WORKSPACE-MAXIMIZE-1 frozen `51272a9`).** FE owns footer pane membership (`visible` / `order` / `footerHeights` px / `fullscreenPaneId`). Ind menu from Manifest HostIDs; persist ∩ manifest. Price always on (not a HostID). **LayoutController** applies CSS Grid, height drag, and legend reorder. `fullscreenPaneId` is workspace-maximize: `applyStack` uses one `minmax(0, 1fr)` track and one participating wrap inside `#charts-stack`. Layout suppression is not `visible[]`. No viewport-fixed overlay. Do not reopen header-offset or z-index-war maximize.
14. **ScaleController (ADR-020 P1).** HostID-based Y-scale prefs (`allowLog`, dormant `scaleGroup`). Price Auto+Log; footers Auto-only. Visibility must not reset prefs. Persisted state must be self-sufficient: Auto OFF without `manualRange` is repaired to Auto ON on restore. **Data paint must not write Auto/Manual/Log** — apply at create, `register`, and explicit pref changes only. Native LWC axis gestures are observation-only (`syncPanePrefsFromChart`): copy Auto/Manual into prefs/UI, never echo `applyOptions` back into LWC.
15. **Scale contribution (ADR-022 / #68 + WOZDUH-PANE-AUTOSCALE-OWNER-1 frozen `1a409e9` + RSX-PANE-AUTOSCALE-OWNER-1 frozen `01e2a7d`).** What Auto *measures* is `autoscaleInfoProvider` from DDR `scaleContribution` (`dynamic` | `bounded` | `ignore`) or pane chrome that reuses the same translator. Pane Auto owners: `candleSeries` → price; `RsxScaleLines` → RSX `[-5,105]`; `WozduhExtremeBands` → Wozduh `[-5,105]`. All DDR RSX/Wozduh plots are `ignore`. LWC 4.2.1 skips a source with null `firstValue()`; RSX chrome is refreshed after DDR `setData`. ScaleController never owns oscillator domains. Do not put Auto back on a hideable plot.
16. **TimeCamera (ADR-021 + ADR-028).** Sole owner of user **VIEW** navigation: ViewIntent (`LIVE` \| `HISTORY`) + ViewGeometry (`centerTime`, `visibleBars`, `barSpacing`, `rightPadding`). Capture / propose / atomic CameraCommit via ChartAdapter. Never indexes market bars (data/compositor resolves time→logical). TF transition protocol → **ADR-029**. Identical CameraCommit is a no-op: after DDR `setData` (full / prepend / indicators), ChartAdapter **force-applies** the canonical logical range to every pane so Wozduh cannot keep an LWC fit-to-content scale.
17. **CrosshairController (ADR-021 P2 + ADR-026 + CROSSHAIR-PANE-HOST-1 + WOZDUH-X-AND-CAMERA-LEAK-1).** Owns `hoveredHostId` + V/H policy only; never timeline. Hover from wrapper pointer events only; LWC move is `{ logical, time? }` (`syncPosition`). ChartAdapter translates: price → `priceSeries`; RSX → `RsxScaleLines.applyCrosshairTime`; Wozduh **X** → `TimelineDecoration.applyCrosshairTime` (full times); Extreme Bands is Y/Auto only. Oscillator DDR plots do not own crosshair. Peers: native vert when time exists, else adapter logical guide; local Y only; no foreign horz.
...
24. **Timeline publish gate.** `WS Connected ≠ History Reconciled ≠ Timeline Publishable`. Mid-session heal follows ADR-017. FE follows TIMELINE-RECOVERY-STATE-1 (current state, not missed edges). Sparse recovery remains ADR-018 MICRO-2B.
...
**FEATURE-TAPE-2 ✅ frozen `61d5ca0`.** Brain V2 sensory runtime: `market.FeatureRuntime2` (thin Jurik/RSTV/ATR wrap, no Frame DAG) + `feature-tape-v2`. Dump API is `DumpFeatureTape2(spec, src15, src1h, src4h)`. V1 `FeatureEvaluator` / `DumpFeatureTape` remain historical only. Star research does not call it.

**STAR-PRICE-PATH-1 ✅ frozen `1317724`.** `market.BuildPricePath` reads archived 15-minute OHLC after a `StarSnapshot`. The Star bar is excluded. Signal entry is that bar's close. Fill entry is the next open, or absent. ATR(14) uses `indicators.ATRSeries` through the Star bar. `ResolveMinuteTouch` orders a later both-touch from 1-minute bars and is not part of the build. No horizon, TP, or SL on `PricePath`. Certified on the snapshot window at read length 16. Digest `40c84ed2c78df3de5ee72ce6ee50730f05b9c621e248559eacfac15a484c7672`. `ATROK` false stores 0, and that 0 is not a volatility.
...
Read-only `a=1-P(TIMEOUT)`, `q=P(TP)/a` on frozen OOF. `a` is a strong resolution ranker. `q` is **FLAT_Q**.

### TP-STOP-INFORMATION-FORK-1 ✅

Locked `TPStopBinaryProbeSpec1` digested **before** the audit. Resolved-only binary Logloss CatBoost on the same 66 facts. Portable evaluator gained a real 2-class consumer (margin + sigmoid; 3-class tests remain green). Audit is descriptive only. Probe pooled Δ vs causal resolved prior **−0.001**. Verdict **FACT_GAP_SUPPORTED**. Next: **TP-STOP-FACT-GAP-1** (target-side room to +2R), not CatBoost retune, not TV-div ignition.
...
| Scale | `ScaleController` = Auto/Manual/Log; `scaleContribution` / pane-host provider = what Auto measures (ADR-022) |
...
**Wozduh:** DAG bus only. Legend = chrome only (no per-tick HTML metrics). **WOZDUH-WIRE-1 frozen** (`0c2ecce`): live/history pack only subscribed Wozduh scalar plot IDs. **WOZDUH-ACTIVE-1A frozen** (`2cd4ca4`): `/api/history` replay runs only the requested Wozduh compute closure; `ReplayClosedBars` default is still compute-all. **WOZDUH-ACTIVE-1B frozen** (`1b724ef`): persistent Frame Wozduh mask is per-TF WS union (Live unused Frames no longer force volume-RSI EMAs for Falcon shadow). **WOZDUH-NUMERIC-NAMES-1:** plot IDs name the math (`woz_vol_rsi_ema12` / `woz_vol_rsi_ema5`, …); values unchanged. **WOZDUH-VOLCROSS-REMOVE-1:** no discrete Wozduh cross event; no `woz_vol_cross` slot/wire. **WOZDUH-COLOR-OVERRIDES-1 frozen** (`1c7ae3d`): factory colors in `ui_config`; sparse `wozduh_color_prefs_v1` + `applyOptions` only (no demand/history). **WOZDUH-STYLE-SECTION-1 frozen** (`b55b64f`): one settings list, checkbox → visibility, color → paint. **HYDRATION-OWNERSHIP-1 frozen** (`c254253`): first `/api/history` after prefs/`requestedPlotIds`; already-hydrated plots are show/hide only; missing columns merge by OpenTime. **CROSSHAIR-PANE-HOST-1 frozen** (`87fd5a8`): oscillator native crosshair is pane chrome (`RsxScaleLines` / `WozduhExtremeBands.applyCrosshairTime`), not `line_rsx` / `woz_vol_rsi_ema5`. Those plots have no crosshair privilege in `needsLwcData`. **CHART-RENDERING-FAMILY-1 frozen** (`01e2a7d`): live indicator paint closed. Native RSX markers stay on `line_rsx` (LWC paint socket); facts stay `wire.Annotation`. Do not start annotation-pane-host work. **WOZDUH-CROSSOVER-PAINT-1 frozen:** closed-bar A×B presentation events (`wozduhCrossovers`) → one overlay above Wozduh strokes. Detection is Go edge-trigger on numeric atoms. Enabled pairs union source plot IDs into existing Wozduh demand. Not a slot, fact, FeatureSpec feature, tape column, or `IndicatorFactEvent`. Do not restore `woz_vol_cross`. **WOZDUH-FAMILY-EYE-1 frozen** (`b9ae83b`): eye batches existing `wozduh_visibility_prefs_live` for two visual families; mute snapshot is restore-only; channel color fields stay behind the channel gear; crossovers are not family members. **CHANNEL-SPLIT-FILLS-1 frozen** (`a02aa98`): `ChannelSeries` optional `fillColor` (upper↔lower) XOR `upperFillColor`/`lowerFillColor` (split at mid). RSI close and Volume EMA5 channel share `boundColor` for both edge strokes. Volume keeps whole-band fill; RSI close uses split fills. Missing key = no region. Do not reopen pane-edge outer fills or twin Volume `upperColor`/`lowerColor`. **WOZDUH-PANE-AUTOSCALE-OWNER-1 frozen** (`1a409e9`): Wozduh Auto domain `[-5,105]` is the Extreme Bands private host; all DDR Wozduh plots `ignore`.

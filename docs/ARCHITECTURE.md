...
**RSX fact visibility (RSX-VISIBILITY-1):** Visibility does not define factual existence. The five families (`rsx_tv_div`, `rsx_tv_pivot`, `rsx_zz_div`, `rsx_fractal_div`, `rsx_fractal_pivot`) coexist as independent facts. Frontend checkboxes only filter paint by annotation `source`. Annotation repaint key is `annotationRevision` + `visibilityMask` (five bits) + `line_rsx` series identity. Unused computation may later sleep based on consumer demand — that is not implemented here.

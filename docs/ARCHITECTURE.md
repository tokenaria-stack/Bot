...
25. **Dense vs sparse live series (MICRO-1.1).** Native and derived time bars expect every bucket. Seconds and ticks do not. FE `appendTick` gap heal is dense-only (`requiresDenseTimeContinuity`). Sparse holes append; they never enter `fe_gapDetected` healing.

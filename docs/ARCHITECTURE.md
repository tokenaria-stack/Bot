...
**RSX TV facts (RSX-SIGNAL-1 / 1.1):** Pine rolling detector emits `rsx_tv_div` (`bullish`/`bearish`) and `rsx_tv_pivot` (`high`/`low`). Times are closed-bar OpenTime ms. Divergence: `ConfirmedAt` = confirm bar, `AnchorAt` = prior bar. Pivot: `ConfirmedAt` = confirm bar, `AnchorAt` = two bars back. Projector paints arrow-only markers. `show_pivots` hides pivot arrows in the FE painter only.

**RSX fractal facts (RSX-SIGNAL-3):** `scanRSXFractalHits` + `CheckClassicDivergence` emit `rsx_fractal_div` (`Pattern` `class_a` / `class_b` / `class_c`) and `rsx_fractal_pivot` (`high` / `low`). Not gated on `div_method`. `AnchorAt` = fractal pivot OpenTime; `ConfirmedAt` = first closed bar that completes the fractal window (`pivot+PivotRadius` for divergence, `pivot+MacroPivotRadius` for P). Class B paints arrow only; A/C add `A Bull` / `A Bear` / `C Bull` / `C Bear`. Fractal pivots are blue arrows. Old L/LL/S/SS/P are not public.

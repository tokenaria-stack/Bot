...
2. **FALCON-REMOVE-1 frozen** — FalconEngine deleted. Canonical RSX/Wozduh are DAG + `indicators`. `ApplyBacktestRSXConfig` remains a legacy-named RSX pin/replay helper. **WOZDUH-NUMERIC-NAMES-1 frozen.** **WOZDUH-VOLCROSS-REMOVE-1 frozen.** **WOZDUH-COLOR-OVERRIDES-1 frozen.** Next: **WOZDUH-STYLE-SECTION-1**. Falcon retirement chapters frozen.
...
**WOZDUH-ACTIVE-1B ✅ frozen** (`1b724ef`). Persistent Frame Wozduh mask = per-TF WS union OR proven internal (`Live`: VolBase|VolRsiEma12|VolRsiEma5). ChartOnly unused Frames are mask 0. **WOZDUH-NUMERIC-NAMES-1 frozen.** **WOZDUH-VOLCROSS-REMOVE-1:** discrete cross event deleted. Do **not** mint a replacement Wozduh fact. **WOZDUH-COLOR-OVERRIDES-1 frozen** (sparse FE colors; factory stays in ui_config). Next: **WOZDUH-STYLE-SECTION-1**.
...
| **VOLUME-SOURCE-ARBITRATION-1** | **FROZEN STEP 2. Verdict VOLUME_SSOT_GREEN_V1.** REST-family SSOT + 1m additive integrity. 18 REST_15M_PARENT + 2 REST_1M_RECONSTRUCTION. Identity `volume-truth:futures-base-v1`. 1m/3m/spot SOURCE_CONFLICT still quarantined. Report `research/volume/VOLUME-SOURCE-ARBITRATION-1.txt`. | Wozduh presentation next: **WOZDUH-STYLE-SECTION-1**. Do not silently refresh v1 from future Binance history. |

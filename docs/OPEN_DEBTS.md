...
| # | Debt | Status | Notes |
|---|------|--------|-------|
| **76** | **ScoreNodes → Forecast engine** | 🟡 **model next** | **DECISION-RESEARCH-1 frozen** `789ddcd` (selector; not eligible for finalization). Holdout remains sealed. **FINAL-DECISION-SPEC-1** only if eligible and explicitly asked. **TARGET-RESOLUTION-2** deferred. |

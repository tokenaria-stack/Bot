...
2. **FRACTAL-MARKER-SSOT-1 GREEN / FROZEN** — unused `ScanRSXMarkers` / display-bar fractal scanner APIs deleted. Production owner remains `FractalFactsAt`. **SCORE-WIRE-1** / **SOCKET-CLEAN-1** / **QDRANT-REMOVE-1** / **INDEX-FOREST-1** frozen. Next cleanup: **FALCON-CONSUMER-MIGRATION-AUDIT-1** (read-only), then **WOZDUH-TRUTH-1**. **ANALOGUE-MEMORY-RESEARCH-1** parked. Untracked: ARTIFACT / QDRANT-CONSUMER / SOCKET-CONSUMER / SCORE-WIRE-CONSUMER / MARKER-OWNERSHIP audits (Git policy later).

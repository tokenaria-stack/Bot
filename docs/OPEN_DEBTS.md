...
| # | Debt | Status | Notes |
|---|------|--------|-------|
| **76** | **ScoreNodes → Forecast engine** | 🟡 **CatBoost path** | **FEATURE-SPEC-2** 64-input contract (Target C). Frozen v1 decision/OOF stack unchanged. Next when asked: **FEATURE-TAPE-2** (not CatBoost, not labels, not holdout). **DECISION-RESEARCH-1 frozen** `789ddcd` (not eligible for finalization). **TARGET-RESOLUTION-2** deferred. |

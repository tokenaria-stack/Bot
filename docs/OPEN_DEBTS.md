...
2. **FALCON-HTF-OSC-DEAD-1 implemented** — deleted unread HTF Falcon oscillator compute (`evaluateHTFOscillators` + `HTFState` RSX/Wozduh floats). Navigator geometry / tracker lifecycle kept. Frame Falcon and `ApplyBacktestRSXConfig` untouched. Next: **FALCON-LIVE-ORPHAN-STREAM-1** (do not merge). Then oracle / unwire / `FALCON-REMOVE-1`. **WOZDUH-TRUTH-1** after Falcon retirement. **ANALOGUE-MEMORY-RESEARCH-1** parked. Untracked: ARTIFACT / QDRANT / SOCKET / SCORE-WIRE / MARKER / FALCON consumer audits (Git policy later). **FRACTAL-MARKER-SSOT-1** / SCORE-WIRE / SOCKET-CLEAN / QDRANT-REMOVE / INDEX-FOREST frozen.

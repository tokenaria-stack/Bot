...
- Next when asked: FORECAST-PROJECTION-C (causal train→val; no global recipe). Do not start it here.
...
- Runtime pin includes SciPy (L-BFGS provenance). SciPy is not part of `ModelSpecDigest`. Logic version not bumped for the pin completion.
...
- Deferred: **TARGET-RESOLUTION-2** (separate 15m→1s TargetSpec). Kill-check GREEN. Do not reopen.

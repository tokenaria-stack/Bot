"""decision-validation-plan-c JSON + DV2C digest. Source is CatBoost OOF logits, not forecast evidence."""
...
FORMAT = "decision-validation-plan-c"
...
def read_plan(path: str) -> Dict[str, Any]:
    ...
    if doc.get("format_version") != FORMAT:
        raise ValueError("decisionplan_c: unknown format")
    if doc.get("decision_validation_logic_version") != LOGIC_V1:
        raise ValueError("decisionplan_c: unknown logic")
    stored = parse_digest_hex(doc["content_digest"])
    body = {k: v for k, v in doc.items() if k != "content_digest"}
    got = hash_plan(body)
    if got != stored:
        raise ValueError("decisionplan_c: ContentDigest mismatch")
    return doc

...
func TestDAGIntraBarImmunity(t *testing.T) {
	...
	sealedVolRsiEma12 := poisonBus.Cur.Get(core.SlotWozduhVolRsiEma12)
	...
	if math.Abs(poisonWoz.VolRsiEma12Value()-sealedVolRsiEma12) > eps {
		t.Fatalf("volRsiEma12 EMA internal poison: restored=%v sealed=%v", poisonWoz.VolRsiEma12Value(), sealedVolRsiEma12)
	}
	...
	checks := []struct {
		...
		{"woz_vol_rsi_ema12", poisonBus.Cur.Get(core.SlotWozduhVolRsiEma12), refBus.Cur.Get(core.SlotWozduhVolRsiEma12)},
		{"woz_vol_rsi_ema5", poisonBus.Cur.Get(core.SlotWozduhVolRsiEma5), refBus.Cur.Get(core.SlotWozduhVolRsiEma5)},
	}

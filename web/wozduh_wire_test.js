...
function mountMixed(factory, events) {
  factory.buildPanes(
    ...
    {
      pane_osc: [
        { id: 'woz_vol_rsi_ema12', hostId: 'wozduh', kind: 'line', renderOptions: { defaultVisible: true } },
...
async function run() {
  await test('A. hidden scalar omitted; woz_vol_rsi_ema5 stays subscribed', () => {
    const factory = new DDRFactory();
    mountMixed(factory, []);
    factory.setSeriesVisible('woz_rsi_close_ema7', false);
    factory.setSeriesVisible('woz_vol_rsi_ema12', true);
    const ids = factory.requestedPlotIds();
    assert.ok(ids.includes('woz_vol_rsi_ema12'));
    assert.ok(ids.includes('woz_vol_rsi_ema5'));
    assert.ok(!ids.includes('woz_rsi_close_ema7'));
    assert.ok(!ids.includes('woz_vol_rsi_ema5_chan'));
    assert.ok(!ids.includes('woz_vol_rsi_ema5_chan_up'));
  });
  ...
  await test('B. visible channel expands to three scalar sources, not compose id', () => {
    ...
    assert.ok(ids.includes('woz_vol_rsi_ema5_chan_dn'));
    assert.ok(!ids.includes('woz_vol_rsi_ema5_chan'));
  });
  ...
  await test('F. omitted live key does not write zero into store', () => {
    ...
    store.applyProjection({
      ...
      plots: { woz_vol_rsi_ema12: [10], line_rsx: [50] },
    }, { commitPaired: true });
    store.appendTick({
      time: 1,
    ...
    assert.strictEqual(store.snapshot().plots.woz_vol_rsi_ema12[0], 10);
  ...
  await test('G. enable scalar fetches then one setData then reveal', async () => {
    ...
    factory.buildPanes(
      ...
      { pane_osc: [{ id: 'woz_rsi_close_ema7', hostId: 'wozduh', kind: 'line', renderOptions: { defaultVisible: false } }] },
    );
    await factory.setSeriesVisible('woz_rsi_close_ema7', true);
    await new Promise((r) => setImmediate(r));
    assert.deepStrictEqual(fetched, ['woz_rsi_close_ema7']);
  ...
  await test('H. enable channel hydrates three sources then one setData', async () => {
    ...
    await factory.setSeriesVisible('woz_vol_rsi_ema5_chan', true);
    await new Promise((r) => setImmediate(r));
    assert.deepStrictEqual(fetched.sort(), ['woz_vol_rsi_ema5_chan_dn', 'woz_vol_rsi_ema5_chan_mid', 'woz_vol_rsi_ema5_chan_up']);
    const sets = events.filter((e) => e.op === 'setData');
    assert.strictEqual(sets.length, 1);
  ...
  await test('I. disable drops unsubscribed woz column; no RSX drop', () => {
    ...
    store.applyProjection({
      times: [1],
      candles: { open: [1], high: [1], low: [1], close: [1], volume: [1] },
      plots: { woz_vol_rsi_ema12: [10], woz_rsi_close_ema7: [3], line_rsx: [50] },
    }, { commitPaired: true });
    store.dropPlotsNotIn(['woz_vol_rsi_ema12', 'line_rsx']);
    const snap = store.snapshot();
    assert.ok(snap.plots.woz_vol_rsi_ema12);
    assert.ok(snap.plots.line_rsx);
    assert.strictEqual(snap.plots.woz_rsi_close_ema7, undefined);
  });

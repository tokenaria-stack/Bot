...
//
//	PhaseConnecting  — WS is up, ticks drain into a bounded buffer (no Frame writes)
//	PhaseLoading     — SQLite + REST recovery hydrate TFrames (Ingress merge, Settled)
//	PhaseReconciling — buffered ticks replay through Runtime.routeTick;
//	                   WS bars land after REST and finalize via the canonical
//	                   live path (AuthorityFinal semantics: WS never loses to REST)
//	PhaseLive        — direct WS → Frame flow (StartDataFeed), loops, strategies
...
// the socket never backpressures while history loads. No Frame is touched.
func (b *BootController) Begin(ctx context.Context) {
	b.setPhase(PhaseConnecting)
	log.Println("[Boot] Phase 0: Connecting — WS ticks buffering, TFrames untouched")
	go func() {
...
// through Runtime.routeTick — the same path live ticks take. Buffered WS
// bars therefore land after REST history and win on conflicts (Final > Settled).
func (b *BootController) Reconcile(m *Runtime) int {
...
func (b *BootController) GoLive() {
	...
	log.Println("[Boot] Phase 3: Live — direct WS → Frame flow")
}

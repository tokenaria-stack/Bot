...
func TestExtractTrendlineData_WicksAndBody(t *testing.T) {
	...
	wHigh, wLow, wTimes := market.ExtractTrendlineData(klines, market.NavigatorTrendWicks)
	...
	bHigh, bLow, _ := market.ExtractTrendlineData(klines, market.NavigatorTrendBody)
...
func TestFindPivots_basic(t *testing.T) {
	...
	ph, pl := market.FindPivots(highs, lows, nil, 2, 1)
...
func TestFindPivots_pivotLow(t *testing.T) {
	...
	_, pl := market.FindPivots(highs, lows, nil, 2, 1)
...
func TestFindPivots_invalidInput(t *testing.T) {
	...
	ph, pl := market.FindPivots([]float64{1, 2}, []float64{1}, nil, 2, 1)
	if ph != nil || pl != nil {
		t.Fatalf("expected nil for mismatched lengths, got %+v %+v", ph, pl)
	}

	ph, pl = market.FindPivots([]float64{1, 2, 3}, []float64{1, 2, 3}, nil, 2, 1)
	if ph != nil || pl != nil {
...
func TestFindPivots_luxAlgoDefaultRight(t *testing.T) {
	...
	ph, _ := market.FindPivots(highs, lows, nil, 10, 1)
	found := false
	for _, p := range ph {
...
func TestNavigatorEngine_isolation(t *testing.T) {
	...
	e1 := market.NewNavigatorEngine(market.NavigatorSettings{SwingLength: 2})
	e2 := market.NewNavigatorEngine(market.NavigatorSettings{SwingLength: 3})
	...
	if len(e1.Markers) == len(e2.Markers) && len(e1.Markers) == 0 {
		...
		if e1.CompletedLines == nil {
			e1.CompletedLines = []market.Trendline{}
		}
	}
...
func TestNavigatorEngine_higherHighCreatesBullLine(t *testing.T) {
	...
	engine := market.NewNavigatorEngine(market.NavigatorSettings{SwingLength: 2, PivotRight: 1})
...
func TestNavigatorEngine_executeEmptySafe(t *testing.T) {
	...
	engine := market.NewNavigatorEngine(market.NavigatorSettings{SwingLength: 10})
...
func TestNavigatorEngine_UpdateMatchesExecute(t *testing.T) {
	...
	settings := market.NavigatorSettings{SwingLength: 2, PivotRight: 1}

	barTimes := market.SynthesizeBarTimesMS(len(highs), 60_000)

	batch := market.NewNavigatorEngine(settings)
	batch.Execute(highs, lows, closes, barTimes)
	batchDTO := batch.GetResultDTO()

	incremental := market.NewNavigatorEngine(settings)
	for i := range highs {
...
func TestNavigatorEngine_GetResultDTO(t *testing.T) {
	...
	engine := market.NewNavigatorEngine(market.NavigatorSettings{SwingLength: 2, PivotRight: 1})
	market.NavigatorBindBarTimesForTest(engine, market.SynthesizeBarTimesMS(20, 60_000))
	engine.Markers = append(engine.Markers, market.TrendlineMarker{
		Index: 5, Price: 100, Text: "HH", Color: "#089981", Type: "Label",
	})
	engine.CompletedLines = append(engine.CompletedLines, market.Trendline{
		X1: 1, Y1: 50, X2: 10, Y2: 60, Color: "#089981", Style: market.NavigatorStyleSolid, IsActive: false,
	})
	engine.Active = &market.Trendline{
		X1: 3, Y1: 55, X2: 12, Y2: 65, Color: "#089981", Style: market.NavigatorStyleSolid, IsActive: true, Bullish: true,
	}
...
func navigatorDTOsEqual(a, b market.NavigatorResultDTO) bool {
...
func TestRunNavigatorAggregator_multiEngineStyles(t *testing.T) {
	...
	merged := market.RunNavigatorAggregator(highs, lows, closes, nil, ui, "15m")
	...
	if styles[market.NavigatorStyleSolid] == 0 ||
		styles[market.NavigatorStyleDashed] == 0 ||
		styles[market.NavigatorStyleDotted] == 0 {
...
func TestBuildNavigatorData_disabledAndRouting(t *testing.T) {
	...
	if got := market.BuildNavigatorData(market.NavigatorUISettings{}, klines, rsx, woz, "", nil, nil); len(got.Lines) != 0 {
		t.Fatalf("disabled navigator should be empty, got %+v", got)
	}

	price := market.BuildNavigatorData(market.NavigatorUISettings{
	...
	rsxNav := market.BuildNavigatorData(market.NavigatorUISettings{
		Enabled: true, Source: "RSX", UseShort: true, ShortLen: 1,
	}, klines, rsx, woz, "15m", nil, nil)
	_ = rsxNav

	wozNav := market.BuildNavigatorData(market.NavigatorUISettings{
...
func runNavigatorThroughBar(settings market.NavigatorSettings, highs, lows, closes []float64, through int) *market.NavigatorEngine {
	engine := market.NewNavigatorEngine(settings)
	barTimes := market.SynthesizeBarTimesMS(len(highs), 60_000)
	for i := 0; i <= through; i++ {
...
func TestNavigatorMomentumFilter_blocksWeakBreak(t *testing.T) {
	...
	base := market.NavigatorSettings{SwingLength: 2, PivotRight: 1}
	...
	weakTimes := market.SynthesizeBarTimesMS(len(highs), 60_000)
	...
	instantTimes := market.SynthesizeBarTimesMS(len(highs), 60_000)
...
func TestNavigatorMomentumFilter_allowsStrongBreak(t *testing.T) {
	...
	highs, lows, closes, breakIdx := buildBearLineSeriesBase()
	settings := market.NavigatorSettings{
		SwingLength:     2,
		PivotRight:      1,
	...
	breakTimes := market.SynthesizeBarTimesMS(len(highs), 60_000)
...
func TestNavigatorTrueRangeAndATR(t *testing.T) {
	...
	tr := market.NavigatorTrueRangeForTest(12, 10, 11)
	if tr != 2 {
	...
	atr := market.NavigatorATRForTest(highs, lows, closes, 2, 2)
	if atr <= 0 {
...
func TestBuildNavigatorResult_timeAnchoredVisuals(t *testing.T) {
	...
	got := market.BuildNavigatorResult(ui, klines, nil, nil, "15m", nil, nil)
...
func TestFindPivots_equalLeftPlateau(t *testing.T) {
	...
	ph, _ := market.FindPivots(highs, lows, nil, 2, 1)
	if len(ph) != 1 || ph[0].Index != 2 || ph[0].Price != 5 {
		t.Fatalf("pivot highs = %+v, want index 2 price 5", ph)
	}
}
...
func TestFindPivots_equalRightRejectsHigh(t *testing.T) {
	...
	ph, _ := market.FindPivots(highs, lows, nil, 1, 1)
	for _, p := range ph {
		if p.Index == 1 {
...
func TestNavigatorSwingScan_luxAlgoFloorIncludesOlderBar(t *testing.T) {
	...
	x, v := market.NavigatorSwingHighForTest(highs, 5, 1)
	if x != 2 || v != 50 {
...
func TestNavigatorSwingLowScan_luxAlgoFloor(t *testing.T) {
	...
	x, v := market.NavigatorSwingLowForTest(lows, 5, 1)
	if x != 2 || v != 5 {
...
func TestSanitizeSlope(t *testing.T) {
	...
	if got := market.SanitizeSlopeForTest(math.NaN()); got != 0 {
		t.Fatalf("NaN slope = %v, want 0", got)
	}
	if got := market.SanitizeSlopeForTest(1e9); got != 100000 {
...
func TestClipNavigatorLinesToChartWindow(t *testing.T) {
	...
	clipped := market.ClipNavigatorLinesToChartWindow(lines, klines)
	if len(clipped) != 2 {
...
func TestRunNavigatorAggregator_TagsInterval(t *testing.T) {
	...
	closes := highs
	barTimes := market.SynthesizeBarTimesMS(len(highs), 4*60*60*1000)
	out := market.RunNavigatorAggregator(highs, lows, closes, barTimes, market.NavigatorUISettings{

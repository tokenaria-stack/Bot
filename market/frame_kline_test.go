...
func warmupMarkerBars(m *Frame, n int, startMs int64, stepMs int64) {
	for i := 0; i < n; i++ {
		base := float64(100 + i)
		m.UpdateKlineTick(exchange.Kline{
...
func TestMarker_UpdateKlineTick_IntraBarDoesNotCompoundFalcon(t *testing.T) {
	...
	history := NewFrame(nil, "1m", ChaosConfig{AOFastPeriod: 5, AOSlowPeriod: 34})
	...
	single := NewFrame(nil, "1m", ChaosConfig{AOFastPeriod: 5, AOSlowPeriod: 34})
	warmupMarkerBars(single, 59, startMs, stepMs)
	single.UpdateKlineTick(final, false)
	wantRSX := markerJurikRSX(single)

	intra := NewFrame(nil, "1m", ChaosConfig{AOFastPeriod: 5, AOSlowPeriod: 34})

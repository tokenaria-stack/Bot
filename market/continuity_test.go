...
// History REST uses a long cold Replay; Live Frame boots on FrameBootKlineLimit=400 only.
...
func TestStateContinuity_HistoryVsLiveBootDepth(t *testing.T) {
	const (
		histBars = 1000
		bootBars = FrameBootKlineLimit // 400
	)
	...
	// ── Path B: Live Boot (short cold Replay — FrameBootKlineLimit) ───────

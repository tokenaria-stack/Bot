...
func TestDetectVolCross(t *testing.T) {
	...
	if got := market.DetectVolCross(40, 50, 55, 48, true); got != "lime" {
		t.Fatalf("bull cross = %q, want lime", got)
	}
	if got := market.DetectVolCross(55, 48, 40, 50, true); got != "red" {
		t.Fatalf("bear cross = %q, want red", got)
	}
	if got := market.DetectVolCross(40, 50, 45, 48, false); got != "" {

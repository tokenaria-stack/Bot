...
// No-op when period is unchanged (ADR-013: never clear runtime without a rebuild path).
func (e *FalconEngine) SetRSXSignalLength(period int) {
	if e.rsxSignal == nil {
		e.rsxSignal = indicators.NewRSXSignalLine(period)
		return
	}
	if period == e.rsxSignal.Period() {
		return
	}
...
// No-op when length is unchanged (ADR-013: never clear runtime without a rebuild path).
func (e *FalconEngine) SetRSXLength(length int) {
	if e.rsx == nil {
		e.rsx = indicators.NewJurikRSX(length)
		return
	}
	if length == e.rsx.Length() {
		return
	}
